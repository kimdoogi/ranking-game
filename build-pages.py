...
T_CLAW = {
  ...
  bump: name => `쿵! ${name} 밑에서 밀려난다`,
  bumpIn: name => `밀려 떨어졌다! ${name} 탈출`,
  bumpBack: name => `${name} 간신히 버텼다`,
  teeter: '걸쳤다!!! 들어가냐 마냐…',
  ...
  bump: name => `Thud! ${name} is shoved out from underneath`,
  bumpIn: name => `Shoved into the chute! ${name} is out`,
  bumpBack: name => `${name} barely hangs on`,
  ...
  bump: name => `咚！${name} 从下面被挤开了`,
  bumpIn: name => `被挤进出口了！${name} 逃脱`,
  bumpBack: name => `${name} 勉强挂住了`,
  teeter: '卡在边上了！！！进还是不进……',
  ...
  bump: name => `ドスン！${name} が下から押し出された`,
  bumpIn: name => `押し出されて脱出！${name}`,
  bumpBack: name => `${name}、かろうじて踏ん張った`,
